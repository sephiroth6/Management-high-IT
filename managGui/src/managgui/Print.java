...
public class Print implements Printable {
    ...
    public int print(Graphics grap, PageFormat pageFormat, int pageIndex) throws PrinterException {
     ...
     private void printDefect(int x, int y, Graphics2D g, Font f, FontRenderContext frc, int type) {
         ...
         switch (type) {
             case Print.DECLARED:
                 s = "Difetto dichiarato: ".concat(this.de.getDeclared());
                 break;
                 
             case Print.FOUND:
                 s = "Difetto riscontrato: ".concat(this.de.getFound());
                 break;
                 
             default:
                 s = "Lavorazione effettuata: ".concat(this.de.getDone());
         }
         ...
         int k = 0, lineLength, n = new BigDecimal(s.length()).divide(new BigDecimal(100), 0, BigDecimal.ROUND_UP).intValue(), j = tokens.length;
         
         for(int i = 0; i < n; i++) {
             
             StringBuilder toWrite = new StringBuilder();
             lineLength = 0;
             while(lineLength < 100 && k < j) {
                 if(lineLength + tokens[k].length() + 1 <= 101) {   
             ...
             g.drawGlyphVector(f.createGlyphVector(frc, toWrite.toString()), x, y + shortHeightLines(i));
     ...
     private void writeCustomer (Graphics2D g, FontRenderContext fRend, Font b, Font f, int x, int y) {
         ...
         g.drawGlyphVector(f.createGlyphVector(fRend, "Cognome: ".concat(this.c.getSurname())), x, y + heightLines(2));
         g.drawGlyphVector(f.createGlyphVector(fRend, "Nome: ".concat(this.c.getName())), x, y + heightLines(3));
         g.drawGlyphVector(f.createGlyphVector(fRend, "Recapito telefonico: ".concat(this.c.getTelephone())), x, y + heightLines(4));
     }
     ...
     private void writeDevice (Graphics2D g, FontRenderContext fRend, Font b, Font f, int x, int y) {
         ...
         if(this.de.getWarranty())
             first = "INFORMAZIONI DISPOSITIVO (IN GARANZIA)";
         else
             first = "INFORMAZIONI DISPOSITIVO";
             
         g.drawGlyphVector(b.createGlyphVector(fRend, first), x, y + heightLines(1));
         g.drawGlyphVector(f.createGlyphVector(fRend, "Modello: ".concat(this.d.getModel())), x, y + heightLines(2));
         g.drawGlyphVector(f.createGlyphVector(fRend, "Categoria: ".concat(deviceType(this.d.getType()))), x, y + heightLines(3));
         g.drawGlyphVector(f.createGlyphVector(fRend, "IMEI/Serial Number: ".concat(this.d.getIdentification())), x, y + heightLines(4));
     }
     ...
     private void writeDetailsIN (Graphics2D g, FontRenderContext fRend, Font b, Font f, int x, int y, String o) {
         ...
         g.drawGlyphVector(f.deriveFont(8,0).createGlyphVector(fRend, "Data e ora ingresso: ".concat(this.r.getDateIn())), x, y + heightLines(2));
         printDeclared(x, y + heightLines(2) + shortHeightLines(1), g, f.deriveFont(8,0), fRend);
         g.drawGlyphVector(f.deriveFont(8,0).createGlyphVector(fRend, "Accessori consegnati: ".concat(o)), x, y + heightLines(2) + shortHeightLines(6));
     }
     ...
     private void writeDetailsOUT (Graphics2D g, FontRenderContext fRend, Font b, Font f, int x, int y, String d) {
         ...
         printFound(x, y + heightLines(2), g, f.deriveFont(8,0), fRend);
         printDone(x, y + heightLines(2) + shortHeightLines(5), g, f.deriveFont(8,0), fRend);
         g.drawGlyphVector(f.deriveFont(8,0).createGlyphVector(fRend, "Data ritiro: ".concat(d)), x, y + + heightLines(2) + shortHeightLines(10));
     }
     ...
     private int writeBillingCustomer (Graphics2D g, FontRenderContext fRend, Font b, Font f, int x, int y) {
         ...
         g.drawGlyphVector(f.createGlyphVector(fRend, this.c.getSurname().concat(" ").concat(this.c.getName())), x, y + shortHeightLines(2));
         g.drawGlyphVector(f.createGlyphVector(fRend, "Codice Fiscale: ".concat(this.bc.getCF())), x, y + shortHeightLines(3));
         g.drawGlyphVector(f.createGlyphVector(fRend, "Partita IVA: ".concat(this.bc.getIVA())), x, y + shortHeightLines(4));
         return writeBillingAddress(customerAddress(this.bc), g, fRend, b, f, x, y + shortHeightLines(5));
     }
